...
async function snapshotSidebar(siteConfig, versionDestPath) {
  const sidebarConfig = siteConfig.themeConfig.nextSidebar;

  return fs.writeFile(
    path.join(versionDestPath, "sidebar.config.json"),
  ...
  if (Array.isArray(sidebarConfig)) {
    return sidebarConfig.map((item) => {
      if (Array.isArray(item)) {
        return [generateVersionedPath(item[0], version, localePath), item[1]];
    ...
    return Object.keys(sidebarConfig).reduce((config, key) => {
      config[generateVersionedPath(key, version, localePath)] =
        sidebarConfig[key];
      return config;
    }, {});
  }
