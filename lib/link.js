...
const { generateVersionedPath } = require("./util");

const indexRE = /(^|.*\/)(index|readme).md(#?.*)$/i;

module.exports = (
  md,
  { externalAttrs, sourceDir, versionedSourceDir, pagesSourceDir, locales }
) => {
  ...
  function toRouterLink(token, link, relPath) {
    ...
    if (!to.startsWith("/")) {
      if (relPath) {
        let version = "next";
        const fullPath = path.resolve(sourceDir, relPath);
        if (fullPath.startsWith(versionedSourceDir)) {
          relPath = path.relative(versionedSourceDir, fullPath);
          const versionPathPattern = /^\/([^\/])/;
          const match = relPath.match(versionPathPattern);
          if (match) {
            version = match[1];
        ...
        for (const locale of locales) {
          if (relPath.startsWith(locale + "/")) {
            localePath = `/${locale}/`;
            break;
          }
        }
        to = generateVersionedPath(to.replace(/^\//, ""), version, localePath);
      } else {
